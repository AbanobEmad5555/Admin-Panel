  ...
  const handleGenerateInvoice = async () => {
    ...
    try {
      ...
      const message =
        ...
        "Failed to generate invoice from POS order.";
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white p-4 shadow-sm">
        <p className="text-sm text-slate-700">
          POS Order ID: <span className="font-semibold">{orderId}</span>
        </p>
      ...
      {isLoading ? (
        <div className="rounded-xl bg-white p-6 text-sm text-slate-500 shadow-sm">Loading order details...</div>
      ) : null}
      {isError ? (
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-6 text-sm text-rose-700 shadow-sm">
          Failed to load POS order details.
        </div>
      ) : null}
      ...
      {data ? (
        <>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <h2 className="mb-3 text-sm font-semibold text-slate-900">Order Overview</h2>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              <DetailField label="POS Order ID" value={order.id} />
              <DetailField label="Temp Order ID" value={order.tempOrderId} />
              <DetailField label="Session ID" value={order.sessionId} />
              <DetailField label="Status" value={order.status} />
              <DetailField label="Payment Status" value={order.paymentStatus} />
              <DetailField label="Created At" value={toDateText(order.createdAt)} />
              <DetailField label="Updated At" value={toDateText(order.updatedAt)} />
              <DetailField label="Refunded At" value={toDateText(order.refundedAt)} />
              <DetailField label="Refund Reason" value={order.refundReason} />
            </div>
          </section>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              <DetailField label="Customer ID" value={order.customerId} />
              <DetailField label="Customer Name" value={order.customerName} />
              <DetailField label="Customer Mobile" value={order.customerMobileNumber} />
              <DetailField label="Loyalty Program Code" value={order.loyaltyProgramCode} />
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
              <DetailField label="Subtotal" value={toCurrency(order.subtotal)} />
              <DetailField label="Tax Amount" value={toCurrency(order.taxAmount)} />
              <DetailField label="Discount Amount" value={toCurrency(order.discountAmount)} />
              <DetailField label="Total" value={toCurrency(order.total)} />
              <DetailField label="Paid Amount" value={toCurrency(order.paidAmount)} />
              <DetailField label="Due Amount" value={toCurrency(order.dueAmount)} />
              <DetailField label="Change" value={toCurrency(order.change)} />
            </div>
          </section>
                ...
                <table className="w-full min-w-[760px] text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-left text-xs uppercase text-slate-500">
                      <th className="py-2 pr-3">Item Image</th>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {payments.length === 0 ? (
              <p className="text-sm text-slate-500">No payment records returned.</p>
            ) : (
              <div className="overflow-x-auto">
          ...
          {additionalFields.length > 0 ? (
            <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <h2 className="mb-3 text-sm font-semibold text-slate-900">Additional Fields</h2>
              <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
