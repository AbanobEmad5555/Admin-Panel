        ...
        <table className={`min-w-[1200px] w-full text-sm ${language === "ar" ? "text-right" : "text-left"}`}>
          <thead className="bg-slate-50 text-slate-900">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold">{language === "ar" ? "حالة الموظف" : "Employee Status"}</th>
              <th className="px-4 py-3 font-semibold">{language === "ar" ? "حالة الحساب" : "Auth Status"}</th>
          ...
          <tbody className="divide-y divide-slate-200">
            {rows.map((row) => (
              <tr key={row.id} className="text-slate-900">
                ...
                <td className="px-4 py-3">
                  {row.authAccount?.staffAccountStatus ? (
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      type="button"
                      disabled={!canEdit}
                      className="rounded-md border border-slate-200 p-2 text-slate-700 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
                      onClick={() => onEdit(row)}
                    ...
                    <button
                      type="button"
                      disabled={!canEdit}
                      className="rounded-md border border-amber-200 p-2 text-amber-700 hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-50"
