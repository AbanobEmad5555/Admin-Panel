...
export default function HiddenModulesList({
  ...
  return (
    <div className="space-y-3">
      ...
      {modules.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-300">
          {labels.empty}
        </p>
      ) : (
        <>
          <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl">
            <table className="w-full table-fixed text-left">
              <thead className="bg-white/[0.06] text-xs font-semibold uppercase tracking-wide text-slate-400">
                <tr>
                ...
                {pageModules.map((module) => {
                  ...
                  return (
                    <tr key={module.moduleId} className="align-top">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-300">{copy.description}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <button
                          ...
                          disabled={isSaving}
                          className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-white/10 bg-white/8 px-3 py-2 text-sm font-medium text-slate-100 transition hover:bg-white/12 disabled:cursor-not-allowed disabled:opacity-50"
                        >
          ...
          {pageCount > 1 ? (
            <div className="flex items-center justify-between gap-3">
              <button
                ...
                className="rounded-xl border border-white/10 bg-white/8 px-3 py-1.5 text-sm text-slate-100 transition hover:bg-white/12 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {labels.previous}
              </button>
